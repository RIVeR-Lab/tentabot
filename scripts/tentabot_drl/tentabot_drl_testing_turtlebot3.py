...
#from openai_ros.task_envs.turtlebot3_world import TurtleBot3WorldEnv
...
if __name__ == '__main__':
    ...
    rospy.init_node('testing_tentabot_rl', anonymous=True, log_level=rospy.WARN)

    mode = rospy.get_param('mode', "")
    benchmark_path = rospy.get_param('benchmark_path', "")
    initial_trained_model_path = rospy.get_param('initial_trained_model_path', "")
    world_name = rospy.get_param('world_name', "")
    velocity_control_msg = rospy.get_param('velocity_control_msg', "")
    odometry_msg = rospy.get_param('odometry_msg', "")
    goal_status_msg = rospy.get_param('goal_status_msg', "")
    max_testing_episodes = rospy.get_param('max_testing_episodes', 0)
    testing_timesteps = rospy.get_param('testing_timesteps', 0)

    deep_learning_algorithm = get_training_param(initial_trained_model_path, "deep_learning_algorithm")
    motion_planning_algorithm = get_training_param(initial_trained_model_path, "motion_planning_algorithm")
    observation_space_type = get_training_param(initial_trained_model_path, "observation_space_type")
    task_and_robot_environment_name = get_training_param(initial_trained_model_path, "task_and_robot_environment_name")
    learning_rate = float(get_training_param(initial_trained_model_path, "learning_rate"))
    n_steps = int(get_training_param(initial_trained_model_path, "n_steps"))
    batch_size = int(get_training_param(initial_trained_model_path, "batch_size"))
    ent_coef = float(get_training_param(initial_trained_model_path, "ent_coef"))
    max_episode_steps = int(get_training_param(initial_trained_model_path, "max_episode_steps"))

    '''
    print("testing_tentabot_rl::__main__ -> deep_learning_algorithm: " + str(deep_learning_algorithm))
    print("testing_tentabot_rl::__main__ -> motion_planning_algorithm: " + str(motion_planning_algorithm))
    print("testing_tentabot_rl::__main__ -> observation_space_type: " + str(observation_space_type))
    print("testing_tentabot_rl::__main__ -> task_and_robot_environment_name: " + str(task_and_robot_environment_name))
    print("testing_tentabot_rl::__main__ -> learning_rate: " + str(learning_rate))
    print("testing_tentabot_rl::__main__ -> n_steps: " + str(n_steps))
    print("testing_tentabot_rl::__main__ -> batch_size: " + str(batch_size))
    print("testing_tentabot_rl::__main__ -> ent_coef: " + str(ent_coef))
    print("testing_tentabot_rl::__main__ -> max_episode_steps: " + str(max_episode_steps))
    '''
    ...
    data_folder_path = benchmark_path + data_folder_tag + "/"
    ...
    testing_input_log_data.append(["benchmark_path", benchmark_path])
    testing_input_log_data.append(["initial_trained_model_path", initial_trained_model_path])
    ...
    testing_input_log_data.append(["testing_timesteps", testing_timesteps])
    ...
    if observation_space_type == "tai2017virtual_FC" or observation_space_type == "Tentabot_FC":
    ...
    elif observation_space_type == "xie2018learning_1DCNN_FC":
    
        policy_kwargs = dict(features_extractor_class=xie2018learning_1DCNN_FC_Policy, net_arch=[dict(pi=[400, 300], vf=[400, 300])],)
        model = PPO("MultiInputPolicy", env, learning_rate=learning_rate, n_steps=n_steps, batch_size=batch_size, ent_coef=ent_coef, tensorboard_log=tensorboard_log_path, policy_kwargs=policy_kwargs, device="cuda", verbose=1)

    elif observation_space_type == "guldenring2020learning_1DCNN_FC":

        policy_kwargs = dict(features_extractor_class=guldenring2020learning_1DCNN_FC_Policy, net_arch=[dict(pi=[400, 300], vf=[400, 300])],)
        model = PPO("MultiInputPolicy", env, learning_rate=learning_rate, n_steps=n_steps, batch_size=batch_size, ent_coef=ent_coef, tensorboard_log=tensorboard_log_path, policy_kwargs=policy_kwargs, device="cuda", verbose=1)

    elif observation_space_type == "patel2021dwarl_2DCNN_FC":

        policy_kwargs = dict(features_extractor_class=patel2021dwarl_2DCNN_FC_Policy, features_extractor_kwargs=dict(features_dim=n_actions),)
    ...
    while(counter < max_testing_episodes):
        ...
        for i in range(testing_timesteps):
            ...
            #print("--------------")
            #print("testing_tentabot_rl::__main__ -> i: {}".format(i))
            #print("--------------")
            ...
            if (done):
                #print("--------------")
                #print("testing_tentabot_rl::__main__ -> Done!")
                #print("--------------")
                counter += 1
                ...
                print("--------------")
                print("testing_tentabot_rl::__main__ -> goal_reached: {}".format(goal_reached))
                print("testing_tentabot_rl::__main__ -> total_time_episode: {}".format(total_time_episode))
                print("testing_tentabot_rl::__main__ -> total_distance_episode: {}".format(total_distance_episode))
                print("--------------")
                ...
                if i == testing_timesteps-1:
                    print("--------------")
                    print("testing_tentabot_rl::__main__ -> Max step has been reached!")
                    print("--------------")
                    counter += 1
