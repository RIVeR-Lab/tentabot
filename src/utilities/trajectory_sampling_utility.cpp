                                                      ...
                                                      string tpitch_samp_type)
{
  ...
  set_trajectory_sampling_dataset_path(tsdataset_path);
  set_trajectory_type(trajectory_type);
...
string TrajectorySamplingUtility::get_trajectory_type()
{
...
void TrajectorySamplingUtility::set_trajectory_type(string new_trajectory_type)
{
  trajectory_type = new_trajectory_type;
}
...
void TrajectorySamplingUtility::construct_trajectory_data_by_simple_car_model(double robot_max_lat_velo, double robot_max_yaw_velo, double dt)
{
  ...
  velocity_control_data_stream.open(trajectory_data_path + "velocity_control_data.csv");
  ...
  vector<double> angular_velocity_samples = angle_sampling_func(2*robot_max_yaw_velo, trajectory_yaw_sampling_count);
  ...
  for (int i = 0; i < lateral_velocity_samples.size(); ++i)
  {
    for (int j = 0; j < angular_velocity_samples.size(); ++j)
    {
      ...
      trajectory_data.push_back( subsample(construct_trajectory_by_model("simple_car", x_init, u, dt), trajectory_sampling_count) );
...
void TrajectorySamplingUtility::read_trajectory_data(string tdata_path)
{
  ...
  ifstream tdata_stream(tdata_path + "trajectory_data.csv");
  ...
  {
    cout << "trajectory_sampling_utility::read_trajectory_data -> Unable to open file!";
  }
...
void TrajectorySamplingUtility::read_velocity_control_data(string tdata_path)
{
  ...
  string::size_type sz;
  string line, spoint, sval;
  ifstream tdata_stream(tdata_path + "velocity_control_data.csv");
  ...
  {
    cout << "trajectory_sampling_utility::read_velocity_control_data -> Unable to open file!";
  }
}
...
void TrajectorySamplingUtility::create_trajectory_data_path()
{
  ...
  else
  {
    ...
    boost::filesystem::create_directory(trajectory_data_path);
  }
}
...
void TrajectorySamplingUtility::save_input_data()
{
  ...
  input_data_stream.open(trajectory_data_path + "input_data.csv");
  ...
  input_data_stream << "trajectory_type" << "," << trajectory_type << "\n";
...
void TrajectorySamplingUtility::save_trajectory_data()
{
  ...
  trajectory_data_stream.open(trajectory_data_path + "trajectory_data.csv");
