...
#include "tentabot_bspline_lee_position_controller_node.h"
...
void BSplineLeePositionControllerNode::PointCallback(const geometry_msgs::PointConstPtr & point_msg) {
  ...
  ROS_INFO("BSPLINE size: %f", b_spline_->size());
...
void BSplineLeePositionControllerNode::OdometryCallback(const nav_msgs::OdometryConstPtr& odometry_msg) {
  ...
  ROS_INFO("local_t: %f", local_t);
  ROS_INFO("maxValidTime: %f", b_spline_->maxValidTime());
...
int main(int argc, char** argv) {
  ros::init(argc, argv, "lee_position_controller_node23");
  ros::NodeHandle nh;

  rotors_control::BSplineLeePositionControllerNode tlee_position_controller_node(nh);
