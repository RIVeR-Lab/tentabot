...
ImageGenerator::ImageGenerator(const ros::NodeHandle& node_handle): nh_{node_handle}
{
  string node_name = ros::this_node::getName();

  // getting params from param server
  nh_.getParam(node_name + "/laser_image_msg", laser_image_msg_);
  nh_.getParam(node_name + "/robot_frame", robot_frame_);
  nh_.getParam(node_name + "/img_width_pos", img_width_pos_);
  nh_.getParam(node_name + "/img_width_neg", img_width_neg_);
  nh_.getParam(node_name + "/img_height", img_height_);
  nh_.getParam(node_name + "/resolution", resolution_);

  cout << "laser_to_image::ImageGenerator -> robot_frame: " << robot_frame_ << endl;
  cout << "laser_to_image::ImageGenerator -> img_width_pos: " << img_width_pos_ << endl;
  cout << "laser_to_image::ImageGenerator -> img_width_neg: " << img_width_neg_ << endl;
  cout << "laser_to_image::ImageGenerator -> img_height: " << img_height_ << endl;
  cout << "laser_to_image::ImageGenerator -> resolution: " << resolution_ << endl;

  // Publishers
  pub_laser_image = nh_.advertise<nav_msgs::OccupancyGrid>(laser_image_msg_, 100);

  // Services
  //get_laser_image_service_ = nh_.advertiseService("get_laser_image", &ImageGenerator::get_laser_image_callback_, this);

  cout << "laser_to_image::ImageGenerator -> Ready to publish laser images..." << endl;
}

void ImageGenerator::laser_scan_callback_(const sensor_msgs::LaserScan& msg)
{
  sensor_msgs::LaserScan scan = msg;
  nav_msgs::OccupancyGrid og_image = generate_image(scan);
  pub_laser_image.publish(og_image);
}

bool ImageGenerator::get_laser_image_callback_(tentabot::get_laser_image::Request& request, tentabot::get_laser_image::Response& response)
{
  nav_msgs::OccupancyGrid og_image = generate_image(request.laser_scan);
  response.laser_image = og_image;
  return true;
}//get_laser_image_callback_

//nav_msgs::OccupancyGrid ImageGenerator::generate_image(sensor_msgs::LaserScan& scan, tentabot::Waypoint& wp)
nav_msgs::OccupancyGrid ImageGenerator::generate_image(sensor_msgs::LaserScan& scan)
{
  //Initializing image
...
void ImageGenerator::add_scan_to_img_(std::vector<int8_t>& image, sensor_msgs::LaserScan& scan)
{
  ...
  try
  {
    tf_.lookupTransform(robot_frame_, 
